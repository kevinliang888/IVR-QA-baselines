...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    # video_feats shape: [1000, 4616, 768]
    video_feats = torch.cat(video_feats,dim=0)
    # video_embeds shape: [1000, 256]
    video_embeds = torch.cat(video_embeds,dim=0)
    # text_embeds shape: [1000, 256]
    ...
    for i,sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        score_matrix_v2t[start+i,topk_idx] = score
    ...
    for i,sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)): 
        ...
        topk_sim, topk_idx = sims.topk(k=config['k_test'], dim=0)
        encoder_output = video_feats[topk_idx.cpu()].to(device,non_blocking=True)
        encoder_att = torch.ones(encoder_output.size()[:-1],dtype=torch.long).to(device,non_blocking=True) 
        ...
        score_matrix_t2v[start + i, topk_idx] = score
    if args.distributed:
        dist.barrier()   
...
def main(args, config):
    ...
    with open(os.path.join(args.output_dir, "test_results.txt"),"a") as f:
