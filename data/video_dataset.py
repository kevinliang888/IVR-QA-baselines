...
class VideoDataset(Dataset):
    ...
    def __init__(self, video_root, ann_root, num_frm=4, frm_sampling_strategy="rand",
                 max_img_size=384, video_fmt='.mp4', filename = 'msrvtt_test.jsonl'):
        '''
        ...
        if filename == 'msrvtt_test.jsonl':
            self.text = [pre_caption(ann['caption'], 40) for ann in self.annotation]
        else:
            self.text = [ann['caption'] for ann in self.annotation]
    ...
    def get_video_path(self, ann):
        ...
        if self.filename == "lsmdc_test.json":
            directory = ann['clip_name'][:ann['clip_name'].rindex('_')]
            video_path = os.path.join(self.video_root, directory, ann['clip_name'] + self.video_fmt)
    ...
    def __getitem__(self, index):
        ...
        if "lsmdc" in self.filename or "activity" in self.filename or "didemo" in self.filename:
            video_path = self.get_video_path(ann)
